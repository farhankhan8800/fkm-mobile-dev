...
const LiveFeed = () => {
 ...
 useEffect(()=>{
  const getData = async ()=>{
    let {data} = await axios.post(livefeed_detailAPI,{
      apiAuth : apiAuth,
      device_type : "4",
  ...
  return (
    <>
      ...
      <div style={{ paddingTop: "56px" }} className="LiveFeed">
        {
          feed_banner ?  <div className="live_feed_banner">
           
          <Image src={feed_banner.cate_image} alt="" width={200} height={120} style={{width:"100%"}} />
         </div> :<Spinner />
        }
         ...
         <div className="top_cashback_store_box">
            ...
            <Swiper className="" 
                ...
                >{
                  topStore ? <>{
                    topStore && topStore.map((item , i)=>{
                      // eslint-disable-next-line react/jsx-key
                      return  <SwiperSlide key={i} >
                        <Link href={`/${item.landing_url}`}>
                        <Image width={200} style={{width:"100%"}} height={90} src={item.image_path} alt='deal of the day' />
                        </Link>
           ...
           <div className="live_feed_wrapper_box">
            {
              feedStore && feedStore.map((item, i)=>{
                // eslint-disable-next-line react/jsx-key
                return( <div className="live_feed_wrapper" key={i}>
                ...
                <div className="live_feed_box" >
                 ...
                 <div className="live_feed_box_bottom">
                  <Image src={item.feed_image} alt="" width={60} height={70} />
                  <p><Link href={item.slug_flag == 0 ? item.landing_url :`/deals/${item.landing_url}` }>Shope Now</Link></p>
                 </div>
                </div>
              </div>)
           ...
           <div className="lodemore_button">
            {
              noData ? "No Data Found":<Button type="button" onClick={lodeMoredata} variant="outlined">Lode More</Button>
            }
